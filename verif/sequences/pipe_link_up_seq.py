...
class pipe_link_up_seq(pipe_base_seq, crv.Randomized):
    ...
    async def config_linkwidth_start_state_downstream(self):
        ...
        async def count_ts1():
            ...
            while  two_ts1s_with_same_link_number_detected == 0:
                ...
                for i in range(len(tses_received)):
                    if(tses_received[i].tses_received == self.pipe_agent_config  and  tses_received[i].use_link_number):
    ...
    async def config_linkwidth_accept_state_downstream(self):
        uvm_root().logger.info(self.name+ " Started config_linkwidth_accept_state_downstream")
        # Update the lane numbers to start with zero and increase sequentially
        for i in range(len(self.tses_sent)):
            self.tses_sent[i].lane_number = NUM_OF_LANES - i - 1
            self.tses_sent[i].use_lane_number = 1
    ...
    async def config_lanenum_wait_state_downstream(self):
        ...
        self.two_ts1s_with_same_lane_numbers_detected = 0
        all_lane_numbers_are_correct             = 0
        uvm_root().logger.info(self.name + " Started config_lanenum_wait_state_downstream")
        pipe_seq_item_h.pipe_operation = pipe_operation_t.SEND_TSES
        pipe_seq_item_h.tses_sent = self.tses_sent
        # S ts1s with the generated lane numbers until two ts1s are received with the same link numbers
        num_of_detected_ts1s_with_same_lane_numbers = 0
        # two_ts1s_with_same_lane_numbers_detected = 0
        

        async def items():
            while (not self.two_ts1s_with_same_lane_numbers_detected):
                await self.start_item(pipe_seq_item_h)
                await self.finish_item(pipe_seq_item_h)
            #uvm_root().logger.info(self.name + "TS1 with the generated Lane numbers sent")
        ...
        async def count_ts1():
            while (not self.two_ts1s_with_same_lane_numbers_detected):
                ...
                for i in range(len(tses_received)):
                    if( not tses_received[i].lane_number == self.tses_sent[i].lane_number  or  not tses_received[i].use_lane_number):
                        all_lane_numbers_are_correct = 0
                    
                
                if (all_lane_numbers_are_correct):
                    self.two_ts1s_with_same_lane_numbers_detected += 1
                    #uvm_root().logger.info(self.name + "TS1 with same Lane numbers Detected")
                else:
                    self.two_ts1s_with_same_lane_numbers_detected = 0
            ...
            if(num_of_detected_ts1s_with_same_lane_numbers == 2):
                self.two_ts1s_with_same_lane_numbers_detected = 1
                #uvm_root().logger.info(self.name + "2 TS1s with the same Link number and Lane number are Detected")
        
        fork1 = cocotb.start_soon(items())
        fork2 = cocotb.start_soon(count_ts1())
        # await Combine(fork1 ,fork2)
        uvm_root().logger.info(self.name + " Finished config_lanenum_wait_state_downstream")
    ...
    async def config_complete_state_downstream(self):
        ...
        # Transmit 16 TS2s until 8 consecutive TS2s are received
        self.eight_consecutive_ts2s_detected = 0

        async def items():
            await self.pipe_agent_config.detected_tses_e.wait()
            for i in range(16):
                self.start_item(pipe_seq_item_h)
                self.finish_item(pipe_seq_item_h)
            self.pipe_agent_config.detected_tses_e.clear()
            ...
            while (not self.eight_consecutive_ts2s_detected):            
                await self.start_item(pipe_seq_item_h)
                await self.finish_item(pipe_seq_item_h)
        ...
        async def count_ts2():
            while (not self.eight_consecutive_ts2s_detected):            
                ...
                for i in range(len(num_of_ts2_received)):
                    if(num_of_ts2_received[i] == 8):
                        self.eight_consecutive_ts2s_detected = 1
                    #uvm_root().logger.info(self.name + "8 Consecutive TS2s with the correct Link and Lane numbers are Detected")
