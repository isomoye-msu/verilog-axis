...
class pipe_speed_change_with_equalization_seq(pipe_base_seq):
    ...
    async def body(self):  # task
        ...
        def recieve_8_ts1s(self):
            ...
            while True:
                ...
                assert(tses_recv[0].ts_type == pipe_operation_t.TS1 and  tses_recv[0].speed_change) 
        ...
        def send_ts2s(self):
            ...
            while True:
                ...
                for i in range(len(tses_send)):
                    ...
                    # sv.cast(tses_send[i].max_gen_supported , self.max_supported_gen_by_dsp)
                    tses_send[i].ts_type = pipe_operation_t.TS2
                    tses_send[i].rx_preset_hint = self.my_rx_preset_hint; 	# as if it is taken  from control register
                    tses_send[i].tx_preset = self.my_tx_preset;				# as if it is taken from control register
                    tses_send[i].equalization_command = 1
                self.send_seq_item(tses_send)
                if(flag):
                    break
        def recieve_8_ts2s(self):
            nonlocal flag
            nonlocal ts_received_count
            while True:
                self.get_tses_received(tses_recv)
                assert(tses_recv[0].ts_type == pipe_operation_t.TS2 and  tses_recv[0].speed_change) 
                # uvm_root().logger.info("pipe_speed_change_without_eq_dsp_seq", "received tses not as expecting step 1")
                ts_received_count += 1
        ...
        fork1 = cocotb.start_soon(send_ts2s())
        fork2 = cocotb.start_soon(recieve_8_ts2s())
        await Combine(fork1,fork2)
        #******************************************************************************************************************************************#
        ...
        async def wait_eios(self):
            nonlocal flag
            await self.pipe_agent_config_h.detected_eios_e.wait()
            flag=1
        
        fork1 = cocotb.start_soon(wait_eios())
        fork2 = cocotb.start_soon(send_eios())
        await Combine(fork1,fork2)
        # wait for TxElecIdle and RxStandby to be asserted
        await self.pipe_agent_config_h.detected_TxElecIdle_and_RxStandby_asserted_e.wait()
        ...
        async def wait_eieos(self):
            ...
            if(self.negotiated_rate<=2):
                await self.pipe_agent_config_h.detected_eieos_e.wait
                flag=1
            else:
                await self.pipe_agent_config_h.detected_eieos_gen3_e.wait
                flag=1
        ...
        match self.negotiated_rate:
            case 1: assert(self.pipe_agent_config_h.new_width == pipe_operation_t.GEN1_PIPEWIDTH)
            case 2: assert(self.pipe_agent_config_h.new_width == pipe_operation_t.GEN2_PIPEWIDTH)
            case 3: assert(self.pipe_agent_config_h.new_width == pipe_operation_t.GEN3_PIPEWIDTH)
            case 4: assert(self.pipe_agent_config_h.new_width == pipe_operation_t.GEN4_PIPEWIDTH)
            case 5: assert(self.pipe_agent_config_h.new_width == pipe_operation_t.GEN5_PIPEWIDTH)
            case _: uvm_root().logger.info(self.name + " " + "negotiated_rate not right")
    
        #assert Rate
        assert(self.pipe_agent_config_h.new_Rate== self.negotiated_rate) ,"Rate signal not right"
        #assert PCLKRate
        assert(self.negotiated_rate == self.calc_gen(self.pipe_agent_config_h.new_width, self.pipe_agent_config_h.new_PCLKRate)) , uvm_root().logger.info(self.name, "PCLKRate signal not right")
        #************************************************************************************************************************************************************************/
        ...
        async def recv_ts1(self):
            ...
            while True:
                self.get_tses_recived(tses_recv)
                if(tses_recv[0].ts_type == pipe_operation_t.TS1 and 	tses_recv[0].ec == 1 ):
                    assert(	(tses_recv[0].ts_type == pipe_operation_t.TS1)   and  (tses_recv[0].lf_value==lf_usp)  and  (tses_recv[0].fs_value==fs_usp) ),  uvm_root().logger.info(self.name + " " + "received tses not as expecting step 4.1")
                    flag=1
                    break
                assert(	(tses_recv[0].ts_type == pipe_operation_t.TS1)   and  (tses_recv[0].tx_preset == my_tx_preset)  and  (tses_recv[0].ec == 0) ) ,  uvm_root().logger.info(self.name +"received tses not as expecting step 4.2")
        ...
        if (not self.enter_phase_2_3):
            ...
            async def recv_ts1s_ec_2(self):
                ...
                while True:
                    self.get_tses_recived(tses_recv)
                    assert(tses_recv[0].ts_type == pipe_operation_t.TS1) , uvm_root().logger.info("pipe_speed_change_without_eq_dsp_seq", "received tses not as expecting step 4.3")
                    if (tses_recv[0].ec == 2):
                        flag = 1
                        break
        ...
        async def recv_ts1s_echo(self):
            ...
            while True:
                self.get_tses_recived(tses_recv)
                assert(tses_recv[0].ts_type == pipe_operation_t.TS1) , uvm_root().logger.info("pipe_speed_change_with_equalization_seq", "received tses not as expecting step 4.4")
                if (tses_recv[0].ec == 3):
                    ts_recived_count += 1
                else:
                    ts_recived_count = 0
        ...
        async def recv_ts1s_options(self):
            ...
            while True:
                ts_recived_count = 0
                self.get_tses_recived(tses_recv)
                assert(	(tses_recv[0].ts_type == pipe_operation_t.TS1) and (tses_recv[0].rcv==0)  and  (tses_recv[0].cursor == self.cursor)  and  (tses_recv[0].pre_cursor == self.pre_cursor)  and  (tses_recv[0].post_cursor == self.post_cursor)  and  (tses_recv[0].ec == 3) ) else uvm_root().logger.info("pipe_speed_change_without_eq_dsp_seq", "")
                assert(pipe_agent_config_h.new_TxDeemph == {self.pre_cursor, self.cursor, self.post_cursor}) , uvm_root().logger.info("pipe_speed_change_with_equalization_seq", "TxDeemph have wrong values ")
                ts_recived_count += 1
                if(ts_recived_count == 2):
                    flag = 1
                    break
    ...
    #*******************************************************step 4.e(gen 2,3,4,5):dsp starts to send TS1 with EC=0 to indicate the of Equalization till it receives two back to back TS1 with EC =0**************************************************/	 
        ...
        async def recv_ts1s_ec_0(self):
            ...
            while True:
                self.get_tses_recived(tses_recv)
                assert(tses_recv[0].ts_type == pipe_operation_t.TS1) , uvm_root().logger.info("pipe_speed_change_with_equalization_seq", "received tses not as expecting step 4.3")
                if (tses_recv[0].ec == 0):
                    ts_recived_count += 1
                else:
                    ts_recived_count = 0
        ...
        async def recv_ts2s_0(self):
            ...
            while True:
                self.get_tses_recived(tses_recv)
                if(tses_recv[0].ts_type == pipe_operation_t.TS2):
                    for i in range(len(tses_recv)):
                        assert(not tses_recv[i].speed_change  and  tses_recv[i].ts_type == pipe_operation_t.TS2) , uvm_root().logger.info("pipe_speed_change_without_eq_dsp_seq", "received tses not as expecting step 5.1")
                    flag = 1
                    break
        ...
        async def recv_ts2(self):
            ...
            while((ts_sent_count < 8)  and  (ts_recived_count < 8)):
                self.get_tses_recived(tses_recv)
                for i in range(len(tses_recv)):
                    assert( not tses_recv[i].speed_change  and  tses_recv[i].ts_type == pipe_operation_t.TS2) ,   uvm_root().logger.info("pipe_speed_change_without_eq_dsp_seq", "received tses not as expecting step 5.2")
                ts_recived_count += 1
    ...
    async def send_seq_item(self,tses):  # task
        ...
        await self.start_item (pipe_seq_item_h)
        await self. await self.finish_item (pipe_seq_item_h)
#  
#async
